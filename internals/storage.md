...
When opening a database file, the following error message may occur:
...
2. Open the database with the latest version of DuckDB, export it to a standard format (e.g., Parquet), then import it using to the old version of DuckDB. See the [`EXPORT/IMPORT DATABASE` statements](../docs/sql/statements/export) for details.
